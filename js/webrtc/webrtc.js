...
var PHONE = window.PHONE = function(config) {
    ...
    var rtcconfig = {
	    ...
	    iceServers : [{ "urls" :
	         navigator.mediaDevices.getUserMedia    ? "stun:stun.services.mozilla.com" :
	        navigator.webkitGetUserMedia ? "stun:stun.l.google.com:19302"   :
	                                       "stun:23.21.150.121"
	    	},
	        {urls: "stun:stun.xten.com"}]*/
  // iceServers:[{"username": "mahxCaxpQf6cSrnc_4vbGpt_kR-s0-iMvi-r58caXHKJ3kBdPPbNr2Vi5FaT2MPYAAAAAF3xB9tzZWllbnNoaQ==",
  // "url": "stun:u3.xirsys.com",
  // "credential": "f0e4ff18-1c28-11ea-b8b6-f676af1e4042"}]
iceServers: [
    {   urls: [ "stun:u3.xirsys.com" ]},
    {   username: "mahxCaxpQf6cSrnc_4vbGpt_kR-s0-iMvi-r58caXHKJ3kBdPPbNr2Vi5FaT2MPYAAAAAF3xB9tzZWllbnNoaQ==",
            credential: "f0e4ff18-1c28-11ea-b8b6-f676af1e4042",
    ...
    function get_conversation(number, isAnswer) {
        var talk = conversations[number] || (function(number){
            var talk = {
                number  : number,
                status  : '',
                image   : document.getElementById('thumbnail-stream'),
                started : +new Date,
                imgset  : false,
                imgsent : 0,
            ...
            // Setup Event Methods
            talk.pc.onaddstream    = config.onaddstream || RTCPeerConnection.ontrack;
    ...
    function onaddstream(obj) {
        ...
        //vid.src = window.HTMLMediaElement.srcObject(stream);
    ...
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    function subscribe() {
	    console.log("webretc Subscribed to " + config.number);
        pubnub.subscribe({
            restore    : true,
    ...
    function onready(subscribed) {
        ...
        console.log('onready myconnection: '+ myconnection);
        connectcb();
        readycb();
    }
