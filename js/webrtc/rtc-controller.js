...
var CONTROLLER = window.CONTROLLER = function(phone, serverFunc){
    ...
    function stream_subscribe(name){
		var ch = (name ? name.toLowerCase() : phone.number()) + "-stream";
		console.log(ch, name, phone.number());
	    pubnub.subscribe({
            channel    : ch,
    ...
    CONTROLLER.joinStream = function(name){
    	...
    	console.log('uniendose al canal:' + name);
	...
	CONTROLLER.isStreaming = function(number, cb){
		console.log('verificando conexion con: '+number);
		console.log(cb);
